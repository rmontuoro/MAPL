module ref_counter_implementation
  use AbstractFreeMemMod ,only : AbstractFreeMemory
  private; public :: ref_counter
  type ref_counter
    private
    integer, pointer :: count => null()
    class(AbstractFreeMemory), pointer :: obj => null()
  contains
...
contains
  function new_ref_counter(object)
    class(AbstractFreeMemory), intent(in) :: object
  ...
  subroutine release(this)
    ...
    if (associated(this%count)) then
      ...
      if (this%count == 0) then
        call this%obj%free_memory(); deallocate (this%count, this%obj)
